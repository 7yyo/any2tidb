...
class SchemaRunner {
    ...
    SchemaRunner(AppConfig config, SchemaExtractor extractor,
    ...
    void run(ApplicationArguments args, List<String> databases, List<String> tables,
             boolean dryRun, boolean dropIfExists, int dbThreads) throws Exception {
        String taskName = App.resolveTaskName(args, driver.type(), "schema");
        ...
        ctx.put("schemaDbThreads", dbThreads);

        TaskManager taskManager = new TaskManager(Path.of("tasks"));
        TaskMeta meta = taskManager.createInteractive(taskName, "schema", driver.type());
        try {
            ...
            steps.add(new SchemaMigrateStep(config, extractor, writer, progress, driver));
